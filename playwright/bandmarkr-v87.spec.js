...
test('v88 keeps the BANDMARKR banner and exposes the simplified installed identity', async ({ page }, testInfo) => {
  ...
  await expect(page.locator('#start-version-refresh')).toContainText('v95');
  await page.screenshot({ path: testInfo.outputPath(`${testInfo.project.name}-v95-bandmarkr.png`), fullPage: true });
});
