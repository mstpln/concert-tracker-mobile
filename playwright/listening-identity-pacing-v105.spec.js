...
test('v106 loads the identity guard without starting provider work', async ({ page }) => {
  ...
  const pacing = await page.evaluate(() => ({
    ...
    recordingModuleLoaded: Boolean(window.BandmarkrListeningIdentityRecordingV106),
  ...
  expect(pacing).toEqual({ loaded: true, installed: true, recordingModuleLoaded: true, recordingOnlyInstalled: true, intervalMs: 2000 });
