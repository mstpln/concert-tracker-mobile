...
v106 is the focused recording-identity correction on draft PR #91 / `fix/v106-defer-musicbrainz-release-context` and remains unmerged. The manual **Complete listening identities** action now processes only followed-band listens whose effective identity still lacks a recording MBID. It reuses the existing trusted-artist requirement, exact normalized ListenBrainz artist/recording validation, 25-combination cap, resumable local cursor, source immutability, additive derived writes and conservative edition rules. Trusted release MBIDs already present are preserved, but release-group-only work is excluded from the manual queue and the action does not call `/musicbrainz/release-context`; release-group enrichment is deferred because it is optional context and not an album-grouping key. The existing Worker route and v105 pacing guard remain available outside this manual action. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v106. Automated validation is synthetic only; no additional live provider call, production R2 operation, production workflow, Worker change or manual deployment has been performed for v106.
