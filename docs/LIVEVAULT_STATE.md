...
Approve scope, use a branch, implement and test, maintain state/build state, push and review a PR, then merge only after explicit `Merge it`. Pull GitHub Desktop only before later local Codex work. The webview-first branch includes deterministic synthetic fixtures, a local fake Worker/storage layer, QA-only build output, desktop/mobile Playwright coverage, a separate manual PWA workflow, isolated offline service-worker validation, PR safety checks, sanitized production-smoke support and continuity documentation. PR QA is green on unit/safety, desktop Chromium and mobile Chromium at head commit `b84de573150eddbcd10ca807c8cf34ddd20a3206`. Automated implementation review is complete. Remaining work before merge is user approval and any optional manual setup or device verification. Physical installation, file pickers, PDF opening, phone storage/permissions, real mobile Chrome behaviour, Cloudflare Pages setup, Worker deployment and production secrets still require manual confirmation.
