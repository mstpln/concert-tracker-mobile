...
v104 / Listening Identity Completion is implemented on draft PR #88 / `feature/v104-listening-identity-completion` and remains unmerged. It adds a manual Settings → Review action that fills missing MusicBrainz recording identity conservatively through the existing ListenBrainz connection, never treats ListenBrainz's release argument as proof of a specific edition, and never invents a release MBID from album text. When a source listen already carries a trusted MusicBrainz release MBID but lacks release-group context, the app can request that exact release's release-group identity through a browser-authenticated, read-only MusicBrainz route added to the existing watched `worker.js`; the route has a 10-second timeout, no hidden retry and performs no R2 write. Canonical sibling listens may contribute only one unambiguous recording MBID and never transfer album-edition identity. Top Albums keeps listen-count-first ranking and remains conservative: partial enrichment alone does not split same-title historical counts, while conflicting trusted same-provider IDs can separate proven editions; release-group identity is context only and never the grouping key. The action is explicitly manual, processes at most 25 unique combinations per run with at least 1,000 ms pacing, writes only additive derived local identity records, and leaves immutable source listening events untouched. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v104. Automated validation on the current branch uses synthetic fixtures only; no live ListenBrainz/MusicBrainz call, Worker deployment, production R2 operation or production workflow has been executed for v104.
