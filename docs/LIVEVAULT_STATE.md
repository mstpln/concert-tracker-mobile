...
v93 is in review on `fix/listening-preparation-recovery` in PR #69. It corrects a physical-device failure found during the first real preparation attempt: when the phone locked or the app was closed, the persisted status could remain **Preparing** even though no work was running. v93 detects abandoned preparation on startup, detects a visible unchanged checkpoint after a conservative five-minute stall window, preserves the migration checkpoint and all derived/source records, shows checkpoint progress, presents **Prepare again**, and requests a screen wake lock where supported. Cleaned totals remain inactive until preparation finishes and the user explicitly activates them. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v93. No production data, R2 object, provider record or source listening observation is changed.

Build 3.3 remains planned to add trusted Spotify links for Top Track and Top Album titles using stored Spotify identity and the existing past-setlist interaction pattern, alongside later grouping and artwork work. It begins only after the v93 recovery correction is reviewed and merged. Offline Concert Access has been permanently removed and is not part of the plan.
