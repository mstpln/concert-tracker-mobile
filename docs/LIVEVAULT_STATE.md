...
v91 / Listening Build 3.2D is implemented for review on `feature/listening-3-2d-review-rollout` in PR #67. It validates conservative duplicate-candidate processing against a generated 250,001-event synthetic archive, applies trusted Levels 1–3 one-to-one, retains probable or ambiguous candidates as derived review records, and writes candidate plans through batches of at most 500 records. Aggregate audit output contains counts only. Settings → Review gains a quiet local listening-data card for unresolved identity and duplicate decisions. **Decide later** is session-only; completed merge, keep-separate, and identity decisions are user-owned and stored only in the disposable derived database. Progress, safe resumption status and version rollback helpers remain local. The v91 shell loads and caches the review module, but no migration or candidate plan runs automatically. No real archive processing, R2 access, provider call, production workflow, deployment, source-observation edit, visible-statistics switch or production-data change is included.

Build 3.3 remains planned to add trusted Spotify links for Top Track and Top Album titles using stored Spotify identity and the existing past-setlist interaction pattern, alongside later grouping and artwork work. It does not begin until Build 3.2D is reviewed and merged and any later real migration or canonical aggregation switch is separately approved. Offline Concert Access has been permanently removed and is not part of the plan.
...
- Probable and ambiguous candidates remain review-only and are stored as derived records, never as source-history edits.
- Candidate persistence is bounded to batches of at most 500 records.
- The Settings review queue includes unresolved, ambiguous and unmatched identities plus probable or ambiguous duplicate candidates.
- **Decide later** does not create a durable decision; completed review decisions are local and user-owned.
...
- Existing screens continue using current source-event aggregation until a later separately approved switch.
