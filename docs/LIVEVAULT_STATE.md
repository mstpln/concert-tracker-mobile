...
Approve scope, use a branch, implement and test, maintain state/build state, push and review a PR, then merge only after explicit `Merge it`. Pull GitHub Desktop only before later local Codex work. The webview-first branch now includes deterministic synthetic fixtures, a local fake Worker/storage layer, QA-only build output, desktop/mobile Playwright coverage, a separate manual PWA workflow, PR safety checks, sanitized production-smoke support and continuity documentation. PR QA is green on unit/safety, desktop Chromium and mobile Chromium. Remaining work is final diff/security/documentation review plus any corrections found there. Physical installation, file pickers, PDF opening, phone storage/permissions, real mobile Chrome behaviour, Cloudflare Pages setup, Worker deployment and production secrets still require manual confirmation.
