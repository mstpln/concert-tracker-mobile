...
v101 is implemented on PR #84 and remains unmerged. It replaces the removed Spotify batch-track metadata request with exact single-track `GET /tracks/{id}` requests, still using only already-stored trusted Spotify track IDs. The manual artwork action processes at most 500 tracks per run, spaces requests by 150 ms, persists local progress every 25 tracks, honors one bounded `Retry-After` wait for HTTP 429, treats an exact 404 as unresolved, and no longer tells a still-connected user to reconnect merely because Spotify returns HTTP 403 for metadata. The existing PKCE authorization, playlist scope, local metadata store, conditional private-R2 write path and immutable source listens remain unchanged. Synthetic unit and browser regressions cover the supported endpoint shape, legacy-v100 connection compatibility, the saved bearer authorization, bounded processing and removal of the obsolete batch request. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v101. No production Spotify call, production R2 write, production workflow, migration or source-listen edit is part of PR #84.
