...
`scripts/ticketmasterConcertAuditV163.js` is a local read-only cleanup audit path. It reads supplied local `concerts.json` and optional `bands.json`, never R2 or provider APIs, and classifies package duplicate groups, provider-attraction conflicts, legacy name-fallback identity records, recoverable/unknown venues, unsafe lifecycle states and lineup-role review cases. Its record-level plan names the retained stable ID, proposed removals, primary/alternate provider evidence, meaningful user-owned field names, unknown field names and whether automatic remediation is safe. A removable package record with `lineupRole: support`, preparation/checklist state, user links, playlist/concert-day progress, tickets, attendance, ratings, notes, photos, setlist data or event grouping requires manual review; a lone lazily defaultable `headliner` value does not disable the clean-package path. Unknown future state also requires manual review. Production cleanup remains a separately authorized next phase after the preventive code is merged and the exact production dry-run is reviewed.
