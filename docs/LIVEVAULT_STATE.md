...
The Data Automation & Enrichment listening-inventory foundation is in progress on `feature/listening-inventory-foundation` / PR #93. The first slice is provider-free: it inventories immutable source listens for current BANDMARKR bands, collapses repeated listens into unique-track work items, prefers exact stored Spotify track IDs, reuses existing MusicBrainz recording identity and stored Spotify metadata before any provider planning, and fails closed on stale band IDs, duplicate band-name mappings or conflicting identities. Aggregate diagnostics contain counts only. Automated QA is synthetic only and makes zero provider calls or production-data operations. The original Spotify Extended Streaming History export was reviewed as an additional read-only reference; its exact `spotify_track_uri` is already retained by BANDMARKR, while its other playback/session fields do not supply missing Spotify artist IDs, album IDs, ISRCs, artwork URLs or MusicBrainz identities and therefore are not being added merely for enrichment.
