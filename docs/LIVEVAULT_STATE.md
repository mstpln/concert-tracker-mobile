...
`workerV158.js` delegates every existing Worker route to `worker.js` and adds only `venues.json`. Venue metadata reads remain authenticated; writes are restricted to the data-maintenance role with existing-style conditional ETag creation/update semantics and document validation. Scheduled automation is intentionally not granted venue-write access in this build. The local `scripts/venueMetadataBackfillDryRun.js` planner can derive conservative unique research targets from supplied JSON files without network/provider/R2 access; it defaults to attended venues and supports an explicitly requested wider local target set.
