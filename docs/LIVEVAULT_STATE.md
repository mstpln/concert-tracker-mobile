...
The selected-year Listening by Genre detail keeps the existing desktop wording and presentation whenever each row fits on one line. Phone layouts through 430px proactively switch to a compact label/value grid so mobile text metrics cannot push the final percentage onto a second line; wider layouts also use compact mode if the full row actually wraps in the original flex-row geometry. Compact mode removes only the repeated word `listens` from non-Total genre rows. Durations, listen counts, time percentages, listen percentages, genre labels and the Total row remain unchanged. A small final font-size reduction is used only if the compact row still needs it.
...
`APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at **v150**. v150 reuses the existing `startStatsV149.js` / `startStatsV149.css` shell assets for the focused Stats responsive correction, so the deterministic shell-file list is unchanged. Focused Playwright coverage verifies the selected-year genre detail at 375px, 414px and 480px, the wider-layout actual-wrap fallback, single-line row containment, compact mobile wording, preserved desktop wording, dark/light presentation and no horizontal overflow. Full desktop/mobile Chromium PR QA remains the merge-readiness gate.
