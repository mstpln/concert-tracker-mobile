...
v91 / Listening Build 3.2D is implemented for review on `feature/listening-3-2d-review-rollout` in PR #67. It validates conservative duplicate-candidate processing against a generated 250,001-event synthetic archive, applies trusted Levels 1–3 one-to-one, and preserves all overlapping probable or ambiguous alternatives as grouped review records. Review groups live in the separate disposable local database `bandmarkr-listening-review-v1`; they never replace or share keys with real canonical source-event records. Settings → Review shows local artist, track, timestamp and source context before a durable duplicate decision. Ordinary unresolved identity baselines are not included in this queue. **Decide later** is session-only; **Keep all separate** records only the group decision, while a manual merge changes only the selected real canonical duplicate relationship. Candidate and review persistence remain bounded to batches of at most 500 records, and aggregate audit output contains counts only. Progress, safe resumption status and version rollback helpers remain local. The v91 shell loads and caches the review module, but no migration or candidate plan runs automatically. No real archive processing, R2 access, provider call, production workflow, deployment, source-observation edit, visible-statistics switch or production-data change is included.
...
- **Keep all separate** changes no canonical source record; a manual merge changes only the selected duplicate's canonical relationship.
