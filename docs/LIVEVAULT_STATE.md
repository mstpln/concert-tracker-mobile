...
The selected-year Listening by Genre detail keeps the existing desktop wording and presentation whenever each row fits on one line. Narrow phone layouts proactively switch to a compact two-column row layout so Android/mobile text metrics cannot push the final percentage onto a second line; wider layouts also use compact mode if the full row actually overflows. Compact mode removes only the repeated word `listens` from non-Total genre rows. Durations, listen counts, time percentages, listen percentages, genre labels and the Total row remain unchanged. A small final font-size reduction is used only if the compact row still needs it.
...
`APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at **v150**. v150 reuses the existing `startStatsV149.js` / `startStatsV149.css` shell assets for the focused Stats responsive correction, so the deterministic shell-file list is unchanged. Focused Playwright coverage verifies the selected-year genre detail at 375px and 480px, single-line row containment, compact mobile wording, preserved desktop wording, dark/light presentation and no horizontal overflow. Full desktop/mobile Chromium PR QA remains the merge-readiness gate.
