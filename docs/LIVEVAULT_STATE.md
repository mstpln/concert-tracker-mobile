...
v106 / Listening Identity Completion correction is merged through PR #91 as main commit `fc3e0a181f02a31ab6fd98ccabae579591fca4ea`. The manual **Complete listening identities** action processes only followed-band listens whose effective identity still lacks a recording MBID. It reuses the trusted-artist requirement, exact normalized ListenBrainz artist/recording validation, 25-combination cap, resumable local cursor, source immutability, additive derived writes and conservative edition rules. Trusted release MBIDs already present are preserved, but release-group-only work is excluded from the manual queue and the action does not call `/musicbrainz/release-context`; release-group enrichment remains optional context rather than an album-grouping key. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v106.

v107 / Data Automation Build A — Foundation is in progress on draft PR #92 / `feature/data-automation-foundation`. It adds the future `DATA_MAINTENANCE_TOKEN` Worker role as a narrow explicit allowlist, contracts for private `listening/track-identities.json` and `weather.json`, additive Spotify listening metadata fields for exact Spotify artist IDs and ISRC, and aggregate-only data-maintenance diagnostics through the existing `UsageTracker`. The maintenance role may read bands, concerts, API usage and the recognized private listening objects, but may write only API usage, Spotify listening metadata, track identities and weather. It cannot access tickets, read/write arbitrary R2 keys, write bands/concerts, or modify the listening manifest, Spotify source archive or ListenBrainz incrementals. Browser access to the new identity/weather documents is read-only. Automated validation is synthetic only. No `DATA_MAINTENANCE_TOKEN` production secret has been created, no production R2 data has been read or written, no provider has been called, no production workflow has run, and nothing has been deployed or merged. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v107 on the branch.

Inspection of the original Spotify Extended Streaming History supplied for this project confirmed that exact Spotify track URIs are already present in the source export and should be reused before any provider lookup. The export also contains playback-behaviour/device/privacy fields such as platform, country, start/end reason, shuffle, skipped and offline state, but no useful ISRC, Spotify artist ID, album ID or artwork fields were observed. Those extra source fields are not being added to BANDMARKR merely because they exist; Build B will reuse exact track IDs and fetch only missing product-useful enrichment.
