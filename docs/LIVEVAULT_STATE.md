...
v107 / Data Automation maintenance contracts are in final review on `feature/listening-maintenance-contracts` / PR #94, now targeting `main` after PR #93 merged. This architectural slice adds an inert-until-configured `DATA_MAINTENANCE_TOKEN` role, a validated conditional `listening/track-identities.json` contract, and additive Spotify metadata support for exact-response Spotify artist IDs and ISRCs. The maintenance role may read bands, concerts, api usage and existing listening objects, but may write only `apiUsage.json`, Spotify listening metadata and track identities; ticket access, news access, `/qa-smoke`, band/concert mutation, listening manifest/archive mutation and the browser-only MusicBrainz route remain forbidden. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v107 and the generated build state reflects v107. No maintenance secret has been created, no production track-identities object exists, no production R2 operation or provider call has been performed, and no schedule or production workflow has been activated. Because `worker.js` is a watched Cloudflare Builds file, an explicitly authorized merge may deploy the reviewed Worker code automatically without authorizing any data or provider operation.
