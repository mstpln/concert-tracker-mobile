...
A controlled five-band provider-identity correction is prepared in PR #76 on `chore/resolve-five-provider-identities`. It contains the exact user-approved MusicBrainz and Spotify artist IDs for The Technicolors, Maudlin Strangers, James and the Cold Gun, The Plan and LE SSERAFIM. The one-time workflow is manual, main-only, serialized with all production writers, and uses a strict ETag write with no merge or hidden retry. It refuses missing or duplicate target names, existing confirmed conflicts and provider IDs already assigned elsewhere. It replaces stale provider-owned identity fields with the approved values while preserving stable IDs, user-owned fields, unrelated provider records and unknown future fields. Merging the PR does not itself run the production update; the separately authorized workflow must still be dispatched after merge.

Later Build 3.3 work remains planned for trusted Spotify links, grouping, artwork and genre correction. General App Updates remain deferred to issue #72. Offline Concert Access has been permanently removed and is not part of the plan.
