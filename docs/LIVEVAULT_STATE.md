...
v103 is implemented on PR #86 / `fix/v103-spotify-rate-limit-feedback` and remains unmerged. The manual artwork action now creates a persisted logical run of at most 100 unresolved trusted Spotify track IDs, spaces calls by 1,000 ms, and saves local metadata after every processed track. If the PWA leaves the foreground, that run is latched as interrupted and cannot silently resume provider calls; the user must explicitly tap **Fetch listening artwork** again. The retry resumes the same persisted run, removes already-resolved IDs from its remaining queue, and therefore cannot turn a 50-of-100 interruption into another 100 Spotify requests. A normal HTTP 429 honors one bounded `Retry-After` wait and then stops if throttling continues; structured `QUOTA_EXCEEDED` stops immediately. Checkpoint-storage failure fails closed before a new Spotify request, and local metadata remains usable if private-data-store synchronization fails. Progress, success, interruption, rate-limit, quota, expired-connection, 403, malformed-response and sync-failure feedback are plain-language and action-oriented. The v102 trusted-ID/relinked-metadata identity rules remain unchanged. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v103. Automated validation uses synthetic Spotify/storage fixtures only; the separately agreed one-time historical artwork backfill remains outside the app build and has not been executed.
