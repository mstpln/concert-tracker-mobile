...
v87 is the active user-facing rebrand build on `style/v87-bandmarkr-rebrand`. The public app name, installed-PWA name and top banner are **BANDMARKR**. The banner retains the existing `#024ddf` blue, height and layout while using the approved centered condensed uppercase wordmark. PWA, maskable and Apple touch icons use the approved blue rounded-square background, white bookmark and blue condensed `BM`. Stable record IDs, user-owned fields, local-storage and IndexedDB data, provider identifiers, remote data, and the existing `concert-tracker-shell-*` cache namespace remain unchanged. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v87.
