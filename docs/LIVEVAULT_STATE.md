...
v103 is implemented on `fix/v103-spotify-rate-limit-feedback` and remains unmerged. The manual artwork action is reduced to at most 100 unresolved trusted tracks per run, spaces calls by 1,000 ms, checkpoints local progress every 10 processed tracks, and preserves any completed local artwork before a provider stop. A normal 429 honors one `Retry-After` wait and then stops with a plain-language temporary-rate-limit message if Spotify still throttles the request. A structured `QUOTA_EXCEEDED` response stops immediately and explains that Spotify's Development Mode quota has been reached, reconnecting will not help, and the user should try again later. Progress, success, 403, expired-connection, malformed-response and generic-provider messages are rewritten to be short and action-oriented. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v103. Automated validation uses synthetic Spotify and storage fixtures only; the separately agreed one-time historical artwork backfill remains outside the app build and has not been executed.
