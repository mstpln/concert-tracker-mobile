...
Ticketmaster standard and VIP/package/premium/lounge/sound-check style listings are classified separately from the physical performance. The shared v163 integrity helper compares band/date, attraction, provider venue or exact location evidence and compatible start time, keeps genuinely distinct same-day performance times separate, and collapses alternate offers to one canonical concert while retaining alternate provider event IDs/URLs as provenance. Re-observed alternate offers merge monotonically so null, empty or partial provider evidence cannot degrade richer stored URLs, titles, status, source or offer classification. The standard Ticketmaster listing is preferred as the primary provider reference when present. Reconciliation applies compatible-time identity both within Ticketmaster data and across Ticketmaster/Tavily-Groq records: materially different known times remain separate, compatible known times plus strong location evidence may enrich the existing stable record, and missing timing/location evidence holds for review. Missing attraction evidence between Ticketmaster records, multiple possible matches and distinct standard listings likewise hold rather than falling through to salted IDs.
...
`scripts/ticketmasterConcertAuditV163.js` is a local read-only cleanup audit path. It reads supplied local `concerts.json` and optional `bands.json`, never R2 or provider APIs, and classifies package duplicate groups, provider-attraction conflicts, legacy name-fallback identity records, recoverable/unknown venues, unsafe lifecycle states and lineup-role review cases. For pre-v163 records without event-title/offer-type metadata, alternate-offer classification requires positive stored evidence from package vocabulary in the ticket URL or an existing `alternateProviderOffers` reference; two otherwise matching legacy listings without that evidence remain a manual ambiguity. Its group-level plan names the retained stable ID, proposed removals, primary/alternate provider evidence, auditable classification reasons, meaningful user-owned field names, unknown field names and whether all removed state is already retained exactly by the canonical record. Conflicting valid lineup roles require manual review in either direction; matching clean headliners or matching support/user state may remain automatic when no state would be lost. Unknown future state remains manual because it has no established merge contract. Production cleanup remains a separately authorized next phase after the preventive code is merged and the exact production dry-run is reviewed.
