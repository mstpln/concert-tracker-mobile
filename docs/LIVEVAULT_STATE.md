...
v98 is the focused listening-tab consistency correction on `fix/v98-listening-tab-consistency` in PR #80 and is awaiting explicit merge authorization. It applies one full-width segmented-control pattern to Band Detail Top Tracks / Top Albums, the Stats Toplist preview, and the dedicated Toplist screen, with shared 42px tab height, padding, radius, active state, typography and a balanced 10px gap before the following timeframe heading or content. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v98. Automated validation uses synthetic desktop and mobile QA only. No production data, provider calls, Worker logic changes, production workflows or deployment are included. The Listening Stats overview card, trusted Spotify track and album links, identity-backed artwork and selected-year genre-detail reconciliation remain deferred to later focused Build 3.3 slices. General App Updates remain deferred to issue #72. Offline Concert Access has been permanently removed and is not part of the plan.
