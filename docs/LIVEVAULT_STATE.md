...
v104 / Listening Identity Completion is implemented on PR #88 / `feature/v104-listening-identity-completion` and remains unmerged. The manual Settings → Review action is limited to listens mapped to followed BANDMARKR bands. A missing MusicBrainz recording MBID is eligible only when BANDMARKR already has trusted MusicBrainz artist identity from the source listen or a confirmed band record; ListenBrainz is accepted only when normalized artist and recording text match and the returned artist MBIDs include that trusted artist identity. ListenBrainz release output is never treated as proof of a specific edition and a missing release MBID is never invented from album text. When a listen already carries a trusted release MBID but lacks release-group context, the app may request only that exact release's release-group identity through the browser-authenticated, read-only MusicBrainz route in the existing watched `worker.js`; the route has a 10-second timeout, no hidden retry and performs no R2 write. Canonical siblings may contribute only one unambiguous recording MBID and never album-edition identity. Top Albums keeps listen-count-first ranking; one partially enriched listen does not split same-title historical counts, while conflicting trusted IDs from the same provider can separate proven editions, and Spotify artwork/links follow that specific edition only when the identity is unambiguous. Each explicit run handles at most 25 unique combinations with conservative provider pacing, stores a resumable local cursor so unresolved early combinations do not starve later work, verifies cursor storage before provider calls, preserves accumulated identity evidence across enrichment and later cleaned-total preparation, and removes that cursor under **Erase this device**. Immutable source events remain untouched. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v104 and generated build state matches. Automated validation uses synthetic fixtures only; no live ListenBrainz/MusicBrainz call, Worker deployment, production R2 operation or production workflow has been executed for v104.
