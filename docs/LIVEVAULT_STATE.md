...
The current merged baseline is **v159 / Venue Metadata Scheduler**. The current implementation branch `fix/concert-ticket-cost-v160` targets **v160 / Concert Ticket Cost Correction**. The manual production venue backfill remains a separate data action, and enabling the scheduled production venue-research lane remains separately authorized after the backfill is validated and uploaded.
...
`APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at **v160** on the ticket-cost correction branch. The deterministic shell is unchanged apart from the release/cache marker. Focused unit coverage proves free-support + paid-headliner, split ticket-price contributions, quantity-conflict handling, malformed-group fail-closed behavior and event/performance stat separation. Full desktop/mobile Chromium PR QA remains the merge-readiness gate.
...
v159 adds only the deferred scheduled venue-enrichment implementation. Tavily/Groq remain shared providers under existing UsageTracker limits; the venue lane is attended-only, explicit-EU-only, capped and evidence-bound. Existing venue display facts are fill-only under automation, conflicts preserve stored facts and `review_needed` remains human-owned. `venues.json` writes remain data-maintenance-only, while `concerts.json` is read-only. The scheduled lane is disabled by repository variable until separately activated after the manual backfill.

v160 changes only read-time grouped ticket-cost interpretation. It preserves every stored ticket price, ticket quantity, concert ID, event relationship, lineup role and unknown field. It performs no production read/write, migration, backfill, provider call or Worker change.

## Safety and release boundary

Automated browser QA uses only synthetic fixtures and the QA fake backend. The v160 implementation branch does not authorize or perform production provider calls, production R2 reads/writes, manual venue backfill, scheduled venue execution, repository-variable activation, Worker deployment, production smoke or deployment. Merging the code would still leave venue automation disabled unless `VENUE_METADATA_RESEARCH_ENABLED` is separately set to `true`; that activation and any required production maintenance credential setup remain explicit later production actions.
