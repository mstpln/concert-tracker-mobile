...
v83 is the active chart-correction build on `fix/v83-listening-chart-scales` and is not merged or deployed. It gives the rolling 14-day views an authoritative daily bucket series with empty dates retained. The yearly listening-hours chart gains a labelled **Listening hours** y-axis and a rounded fixed maximum derived from the complete history for the selected genre, so moving among six-year windows does not rescale the chart. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v83.
