...
v105 is the focused MusicBrainz pacing correction on draft PR #90 / `fix/v105-musicbrainz-pacing` and remains unmerged. It adds a browser-only pacing guard for the existing authenticated `/musicbrainz/release-context` route. MusicBrainz release-context request starts are reserved at least 2,000 ms apart, including overlapping calls, while ListenBrainz and unrelated browser fetches are untouched. A provider rate-limit response still stops the identity run with no automatic retry. The v104 identity rules, 25-combination manual cap, source immutability, R2 boundaries, Worker route and album-edition policy are unchanged. `APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at v105. Automated validation is synthetic only; no additional live provider call, production R2 operation, production workflow or manual deployment has been performed for v105.
