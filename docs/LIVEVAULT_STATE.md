...
The selected-year Listening by Genre detail keeps the existing desktop wording and presentation whenever each row fits on one line. When the available row width would cause wrapping, the detail switches to a compact two-column row layout and removes only the repeated word `listens` from non-Total genre rows. Durations, listen counts, time percentages, listen percentages, genre labels and the Total row remain unchanged. The compact mode is responsive to actual available width rather than device identity, and uses only a small final font-size reduction if the compact row still needs it.
...
`APP_VERSION` and `CACHE_NAME_LITERAL` are synchronized at **v150**. v150 reuses the existing `startStatsV149.js` / `startStatsV149.css` shell assets for the focused Stats responsive correction, so the deterministic shell-file list is unchanged. Focused Playwright coverage verifies the selected-year genre detail at 375px and 480px, single-line row containment, compact mobile wording only when needed, preserved desktop wording, dark/light presentation and no horizontal overflow. Full desktop/mobile Chromium PR QA remains the merge-readiness gate.
