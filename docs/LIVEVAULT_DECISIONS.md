...
**Consequence:** The existing v114 album priority, UsageTracker accounting, DAB6 Spotify circuit, DAB7 global lease, conditional metadata writes, band-ownership checks, exact-track provider seeds, provider ownership, unknown-field preservation and immutable source observations remain authoritative. An unchanged caught-up manifest performs no full-history reconstruction and zero Spotify calls; a changed manifest permits full planning under the normal gates. Definitive exact-track no-match or no-usable-artwork results are durably suppressed for that album-group key and representative Spotify Track ID, while temporary failures remain retryable. The lower-level manual album runner retains its separate ceilings and authorization. The scheduler assumes one trusted host because its due and rolling-budget marker is private and local. Installing or activating that scheduler, setting scheduled credentials or authorization, reading private production listening data, calling Spotify, writing usage/artwork metadata, running a production workflow or deploying remain separate production actions. DAB8 is Node/trusted-local plumbing, so v115 PWA and deterministic build facts remain unchanged.
