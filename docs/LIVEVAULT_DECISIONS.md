...
**Consequence:** The browser may conditionally publish `listening/band-activity.json`, a validated derived object containing only stable band IDs, exclusive activity-window counts, per-bucket latest timestamps, aggregate freshness and a catalogue fingerprint. The automation role receives GET-only access to that exact object and remains forbidden from every raw listening archive, manifest and other listening-derived object. The lane fails closed on missing/stale/mismatched aggregate data, duplicate identities, exact response-ID mismatch, malformed image arrays, changed latest state or ETag conflict. Empty exact image arrays are a valid terminal no-image outcome for that trusted identity rather than a refresh schedule; an identity change makes it eligible again. Network/5xx failures receive one bounded retry and remain retryable, while 429/quota uses DAB6 and stops further work. Images are stored only under the trusted Spotify provider record; manual `photoUrl` is never written. DAB7, UsageTracker/pacing, provider ownership and the existing workflow schedule remain unchanged. Creating the first production aggregate, running the workflow, calling Spotify, deploying the Worker, merging or changing production data remain separate actions.
