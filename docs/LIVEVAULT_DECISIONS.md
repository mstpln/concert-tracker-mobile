...
## 2026-08-13 — Scheduled Spotify album artwork is trusted-local, daily and lease-admitted

**Decision:** DAB8 keeps automatic Spotify listening-artwork maintenance on one trusted local host rather than moving private listening reads into GitHub Actions. A scheduled opportunity may be admitted no more often than every 24 hours, uses one fixed private local state file at `.livevault-maintenance/spotify-album-artwork-schedule.json`, and may process at most 25 album groups. A due candidate must acquire the existing DAB7 persisted provider lease and recheck the local due marker under that lease before `lastAttemptAt` is advanced. A busy lease is a deferral rather than a consumed daily attempt; once admitted, an attempt is recorded before private/provider work, so a later failure waits for the next daily opportunity.

**Reason:** The album-artwork source requires the browser credential and must retain the trusted-local private-data boundary. The schedule also needs to avoid two separate failure modes: a host wake loop must not create rapid provider retries, and contention with another provider process must not waste the only daily artwork opportunity before the artwork run actually starts. A fixed state path and a 25-group scheduled ceiling prevent alternate local state files or a larger manual-maintenance ceiling from becoming ways to manufacture extra scheduled quota.

**Consequence:** The existing v114 album priority, at-least-1,000-ms pacing, UsageTracker accounting, DAB6 Spotify circuit, DAB7 global lease, conditional metadata writes, band-ownership checks, exact-track provider seeds, provider ownership, unknown-field preservation and immutable source observations remain authoritative. The lower-level manual album runner retains its separate 100-group hard ceiling, but DAB8 scheduled execution cannot exceed 25. The host scheduler may wake DAB8 more often than daily, but fresh runs take no lease and perform no production work; busy-lease wakes may retry later without consuming the daily attempt. The existing album runner executes inside the already-held DAB8 lease without reacquiring it. Installing or activating the host scheduler, setting the scheduled-production confirmation, reading private production listening data, calling Spotify, writing usage/artwork metadata or deploying remain separate production actions. DAB8 is Node/trusted-local plumbing, so v115 PWA and deterministic build facts remain unchanged.
