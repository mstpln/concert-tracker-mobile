...
**Reason:** The separately authorized long-running v111 backfill stopped after 97 persisted provider steps on `musicbrainz:error`. Under the old policy, a temporary provider outage could both halt the invocation and permanently remove that track from automatic routing. A transient availability failure should remain fail-closed for the current invocation without becoming a permanent data-quality judgment.

**Consequence:** A transient MusicBrainz result is persisted as `retry` and still stops the current bulk invocation. Legacy recovery derives `nextEligibleCheckAt` from the original `checkedAt` plus 30 minutes, preserves unrelated and unknown fields, and leaves incomplete or non-transient errors terminal. Any legacy correction is written through a strict identity-only conditional persistence step before provider usage is reserved or a provider call is made; concurrent identity changes abort the run. Spotify, ListenBrainz, 404 fallback, review quarantine and the focused diagnostic path remain unchanged. Resuming production remains separately authorized after review and merge.
...
**Reason:** The first post-PR-#100 production attempt persisted 118 provider steps and correctly converted the transient MusicBrainz condition into `musicbrainz:retry`, but stopping the entire 12,000-track migration at every transient provider retry would require repeated manual restarts and leave unrelated provider work idle. Deferring the affected provider preserves the cautious interpretation of 429/503/network conditions without treating one transient response as a reason to stop other provider families.

**Consequence:** Bulk mode carries the deferred-provider set across internal 100-step chunks. When only work for deferred providers remains, the invocation stops with `provider_retry_wait:<provider>`; when no provider step is currently eligible but dated retry state remains, it reports `retry_wait`. True provider-wide halts, terminal errors, usage denial, stale state, concurrency changes and persistence failures remain global stops. Retry state and `nextEligibleCheckAt` remain durable and authoritative, no hidden retry loop is added, and production resumption still requires separate authorization after merge.
