...
**Consequence:** Package duplicates retain one canonical stable BANDMARKR ID and alternate provider provenance. Provider-attraction conflicts and legacy name-fallback records are classified for cleanup/review. Legacy package classification requires positive stored evidence rather than treating absent v163 metadata as standard/alternate by default. Cleanup safety is evaluated across the full group: meaningful user state on a removed record is safe only when the canonical record already retains the exact value, conflicting valid lineup roles require manual review in either direction, and unknown future fields remain protected because no merge contract exists for them.
