...
**Decision:** Historical listening identity resolution moves toward the cache/catalogue-first architecture from the revised Data Automation plan: reuse existing recording identity first, anchor automatic local matching to the already trusted MusicBrainz artist MBID, require exact normalized recording evidence and conservative explicitly owned release evidence, normalize catalogue pages through sequential pure checkpoints before future persistence, leave ambiguous candidates unresolved, derive trusted Spotify track URLs locally without provider calls, and reserve Spotify for genuinely Spotify-specific presentation or metadata work. Existing durable root or provider `needs_review`, `retry`, `error`, and `no_match` states are not automatically reopened by the C2 resolver or future batch-bridge planning.

**Reason:** The revised architecture should reduce provider dependence without discarding review decisions, retry ownership, terminal quarantine, provider boundaries, or previously persisted maintenance evidence. A new evidence source is not sufficient reason to silently bypass durable state, and pagination completion for one MusicBrainz release-browse scope is not sufficient proof that every recording credited to the trusted artist has been represented.
...
## 2026-08-12 — Setlist enrichment marks absence only after trustworthy provider outcomes

**Decision:** Scheduled historical setlist enrichment may advance `setlistCheckedAt` only after setlist.fm returns a trustworthy `found` or definitive `no_match` outcome. Network, HTTP, malformed-response, usage-cap and MBID-conflict outcomes remain retryable. Historical Spotify song-link enrichment similarly sets `spotifyChecked` only after a trustworthy track match or definitive no-match; token, network, HTTP, malformed-response and usage-cap failures stop the current song-link pass while leaving the failed and later songs unchecked.

**Reason:** The previous null-based contracts collapsed genuine absence and transient provider failure into the same value. That could suppress a failed setlist lookup for 30 days or mark a song permanently checked after a temporary Spotify failure.

**Consequence:** Genuine setlist absence keeps the existing 30-day recheck policy, while provider trouble remains eligible on a later scheduled run without hidden retries. Successful Spotify progress earlier in a pass is preserved, but failed and later songs remain retryable. `concerts.json` is written for enrichment only when trusted pipeline fields actually changed. Provider schedules, caps, stable IDs, user-owned fields, unknown future fields and provider ownership boundaries remain unchanged. DAB4 is Node-only, so `APP_VERSION`, `CACHE_NAME_LITERAL` and deterministic build-state facts remain at v115. This decision does not authorize a live provider call, production workflow, production data write, deployment or merge.

## 2026-08-12 — Scheduled MusicBrainz research is a bounded due-work queue

**Decision:** Keep MusicBrainz inside the existing Monday/Wednesday/Friday structured research workflow, but select scheduled MusicBrainz provider operations from current due state rather than consuming the shared cap in band-array order. Bootstrap, incomplete and retry work outranks routine retained refreshes; within a priority class, selection prefers bands that have not yet received a slot and then the oldest-due work with deterministic tie-breaking. The existing five-call per-run ceiling and two-second UsageTracker pacing remain authoritative.

**Reason:** The structured run can have both artist-metadata and release-group work due for the same early band. Letting those calls consume the shared cap before later bands are considered creates avoidable starvation, while forcing MusicBrainz onto the same three-day refresh cadence as Spotify creates more fixed-schedule polling than the provider needs. A small state-derived queue preserves useful retained coverage while reducing unnecessary polling and spreading scarce calls fairly.

**Consequence:** `preloadStructuredRun.js` no longer overrides MusicBrainz release refresh to three days; MusicBrainz uses the base configured retained interval while Spotify keeps its three-day cadence. Only trusted unique MBIDs are schedulable; duplicate trusted MBIDs fail closed without consuming safe planner capacity. If the scheduler cannot read the latest band state, MusicBrainz is skipped for that run instead of failing the research pipeline. Manual MusicBrainz identity backfill, C3/C4 private listening catalogue/history maintenance, provider ownership, stable IDs, user fields and unknown future fields are unchanged. DAB5 is Node-only, so v115 and deterministic build facts remain unchanged. This decision does not authorize a live provider call, production workflow run, production data write, deployment or merge.

## 2026-08-13 — Node Spotify rate and quota safety uses one persisted shared circuit

**Decision:** Scheduled Node research and trusted-local Spotify artwork maintenance share one additive persisted circuit at `apiUsage.json.spotify.circuit`. A usable positive Spotify `Retry-After` is honored with a one-second safety margin; missing, blank, zero, negative or unusable retry timing uses a conservative 30-minute fallback. Explicit `QUOTA_EXCEEDED` opens a 24-hour BANDMARKR internal cooldown before a later probe. That 24-hour value is a project safety/probe interval, not a claim that Spotify resets quota after 24 hours. Browser Spotify OAuth remains outside this Node circuit.

**Reason:** Spotify Development Mode quota is shared more broadly than a single Node invocation, so independent scheduled and trusted-maintenance callers must observe the same durable provider-safety state. At the same time, Spotify does not document a quota-reset time that BANDMARKR can safely encode as fact, and application-token authentication success does not prove the Web API quota has recovered.

**Consequence:** Malformed persisted circuit state fails closed. Circuit updates preserve unknown future fields, do not shorten an active block, and do not downgrade an active quota block to an ordinary rate-limit block. Changed safety state is persisted immediately and persistence failure fails closed. An expired circuit closes only after an actual eligible Spotify Web API request succeeds; cached/skipped work and successful token authentication do not close it. Existing schedules, caps, pacing, provider ownership, conditional writes, stable IDs, source observations, C3/C4 recording identity and v115 PWA/build facts remain unchanged. DAB7 separately owns cross-scheduler lock/lease behavior, so two independent Node schedulers may still both begin a probe if they observe the same expired circuit concurrently. No live provider call, production workflow, deployment or production-data operation is authorized by this decision.

## 2026-08-13 — Node provider schedulers use one persisted cross-environment lease

**Decision:** Provider-using GitHub workflows and trusted-local Spotify artwork maintenance must acquire one shared lease at `apiUsage.json.schedulerLease` before provider work. Acquisition is an ETag-strict compare-and-set against the latest usage document. Active or malformed lease state fails closed. Lease ownership uses a unique lease ID, and release may clear only that same ID after rereading the latest state. The default lease expires after six hours; DAB7 does not add a heartbeat.

**Reason:** GitHub Actions concurrency serializes GitHub jobs but cannot serialize a separately started trusted-local Node process. Without a persisted cross-environment guard, independent processes can race usage-state writes and DAB6 Spotify circuit probes even when each process is individually quota-aware.

**Consequence:** Structured research, focused Tavily/Groq research, MusicBrainz identity backfill, provider identity backfill, Spotify candidate acquisition and setlist-insights backfill retain their existing `live-vault-data-writes` GitHub Actions concurrency while also using the persisted lease. Trusted-local Spotify track-artwork and album-artwork production use the same lease after their existing authorization gates. A busy or malformed lease stops before provider work; a killed owner may conservatively block later provider work until the six-hour expiry rather than allowing overlap. Existing schedules, caps, pacing, retries, provider ownership, conditional writes, stable IDs, source observations, browser Spotify OAuth, C3/C4 recording identity and v115 build facts remain unchanged. This decision authorizes no live provider call, production workflow, deployment or production-data operation.

## 2026-08-13 — Scheduled Spotify album artwork is trusted-local, bounded and lease-admitted

**Decision:** DAB8 keeps automatic Spotify listening-artwork maintenance on one trusted local host rather than moving private listening reads into GitHub Actions. A scheduled opportunity may be admitted no more often than every four hours, uses one fixed private local state file at `.livevault-maintenance/spotify-album-artwork-schedule.json`, processes at most five unresolved album groups, spaces Spotify track requests by at least 5,000 ms, and reserves no more than 30 scheduled Spotify track lookups in any rolling 24-hour window. A due provider candidate must acquire the DAB7 persisted provider lease and recheck the local due marker under that lease before `lastAttemptAt` and its conservative provider reservation are persisted. A busy lease is a deferral rather than a consumed attempt.

**Reason:** The album-artwork source requires the browser credential and must retain the trusted-local private-data boundary. The fixed state path, four-hour due marker, small per-run cap, conservative rolling reservations and slower pacing prevent host wake loops, interruption and alternate local state files from manufacturing extra scheduled quota. A caught-up scheduler must also avoid reconstructing the complete private archive when its lightweight listening manifest has not changed.

**Consequence:** The existing v114 album priority, UsageTracker accounting, DAB6 Spotify circuit, DAB7 global lease, conditional metadata writes, band-ownership checks, exact-track provider seeds, provider ownership, unknown-field preservation and immutable source observations remain authoritative. An unchanged caught-up manifest performs no full-history reconstruction and zero Spotify calls; a changed manifest permits full planning under the normal gates. Definitive exact-track no-match or no-usable-artwork results are durably suppressed for that album-group key and representative Spotify Track ID, while temporary failures remain retryable. Browser normalization and writes preserve those provider-owned suppressions and unknown root fields, while current remote suppression removal wins over stale browser-local state. The lower-level manual album runner retains its separate ceilings and authorization. The scheduler assumes one trusted host because its due and rolling-budget marker is private and local. Installing or activating that scheduler, setting scheduled credentials or authorization, reading private production listening data, calling Spotify, writing usage/artwork metadata, running a production workflow or deploying remain separate production actions. This browser metadata compatibility correction bumps the synchronized PWA/cache/build facts to v116.

## 2026-08-15 — Settings uses one authoritative renderer with safety at action boundaries

**Decision:** v123 Settings has one authoritative renderer for the **Automation**, **Review**, and **Data** tabs. Review contains only decisions that require the user; technical listening preparation, identity completion, artwork maintenance, import/recovery and device actions belong under Data, with maintenance/recovery collapsed by default. Legacy Settings injectors do not regain ownership by recreating their old DOM anchors. When a later module previously enforced corrected behavior through an old Settings click/DOM takeover, that behavior must remain available at the underlying action API boundary instead.

**Reason:** The accumulated layered Settings presentation had multiple modules observing and rewriting the same subtree. Rebuilding a coherent Settings system while leaving safety corrections dependent on retired selectors would make the UI look unified but silently reopen older provider behavior. The v123 CI cycle exposed this specifically for v106 recording-only identity completion and v101 exact-track Spotify artwork enrichment.

**Consequence:** `settingsV123.js` is the sole source of Settings DOM. Existing data/storage/action modules remain reusable, stable IDs and user/provider-owned fields remain unchanged, and later safety semantics survive the presentation rewrite without duplicate legacy UI. In particular, manual recording identity stays ListenBrainz recording-only with MusicBrainz release-group work deferred, and browser Spotify artwork remains exact-track, resumable, trusted-ID preserving and quota-aware. Future Settings presentation changes should call the current action contract rather than depend on an older presentation layer's selectors or click interception.

## 2026-08-15 — Listening aliases extend local band-name mapping without asserting provider identity

**Decision:** A band may carry an optional additive `listeningAliases` string array. For local listening attribution, the canonical band name and those aliases form one normalized name set. A normalized value maps automatically only when exactly one stable BANDMARKR band ID owns it; alias-vs-alias and canonical-vs-alias collisions across bands are ambiguous and remain unmapped. An explicit known `bandId` or `localBandId` remains authoritative.

**Reason:** Artist display names can change while historical source listens retain the earlier credit. BANDMARKR needs to keep those observations attached to one stable local artist without rewriting immutable listens or pretending a text alias proves Spotify, MusicBrainz, or another provider identity.

**Consequence:** Aliases affect only local BANDMARKR listening attribution and downstream derived/artwork grouping that depends on that attribution. They do not create or replace provider IDs, do not rewrite source observations, do not override user-reviewed identity decisions, and do not weaken same-name ambiguity rules. Older band records without `listeningAliases` remain valid. The first intended use is a future Bea record whose current canonical name is `Bea` and historical listening alias is `Bea and her Business`; adding that production band record remains a separate production-data action.

## 2026-08-16 — Missing artist images use a privacy-safe, exact-identity maintenance lane

**Decision:** Add artist-image maintenance to the existing Monday/Wednesday/Friday structured research workflow as its final, lower-priority provider lane. Eligibility is based on the established visible-image helper before the ten-band cap: any usable manual, trusted Spotify or trusted official-site image excludes the band. Trusted Spotify identities go directly to exact artist lookup by ID. Bands without a trusted Spotify ID use the existing conservative identity workflow first and receive no trusted artwork until exact lookup confirms that newly trusted ID. Spotify artist-search thumbnails do not become trusted provider artwork.

**Reason:** The prior structured identity resolver correctly reused a trusted Spotify ID and therefore made no provider call, but that also meant an identity with no images could remain permanently image-less. Priority needs listening activity, while raw listening history must remain unavailable to ordinary GitHub provider automation.

**Consequence:** The browser may conditionally publish `listening/band-activity.json`, a validated derived object containing only stable band IDs, exclusive activity-window counts, ordinal recency ranks, aggregate freshness and a catalogue fingerprint. Exact listen timestamps remain in private browser storage. Root, record and bucket keys are strictly allowlisted on both write and read; the automation role receives GET-only access to that exact validated object and remains forbidden from every raw listening archive, manifest and other listening-derived object. The exact current band-ID set and fingerprint bind valid ranking data to the catalogue. Missing, stale, malformed or mismatched aggregate data falls back to deterministic no-listening ordering so eventual maintenance continues without inventing listening priority. Duplicate identities, exact response-ID mismatch, malformed image arrays, changed latest state or ETag conflict fail closed. Empty exact image arrays are a valid no-image outcome retried after 180 days, or immediately after identity change; this is not a refresh path for bands that already have usable artwork. Network/5xx failures receive one bounded retry and remain retryable, while 429/quota at identity or image lookup uses DAB6 and stops further work. Images are stored only under the trusted Spotify provider record; manual `photoUrl` is never written. DAB7, UsageTracker/pacing, provider ownership and the existing workflow schedule remain unchanged. Creating the first production aggregate, running the workflow, calling Spotify, deploying the Worker, merging or changing production data remain separate actions.

## 2026-08-17 — v135 retires the active release-alert product path and makes scheduled release discovery inert

**Decision:** v135 retires Releases as an active user-facing alert/feed surface and retires scheduled release-alert planning. Alerts is concert-only. The Monday/Wednesday/Friday structured preload sets `structuredReleaseMonitoringEnabled` to false before research runs and replaces lifecycle release-alert planning with an empty plan, so scheduled release catalogue discovery and new lifecycle release alerts are not produced. Existing stored release research, lifecycle fields, compatibility data and unknown fields are preserved rather than destructively migrated. Spotify remains available only where separately justified by non-release product features and existing provider-safety contracts.

**Reason:** The active release product path was no longer wanted, while leaving its scheduled discovery/planner wiring executable would continue spending provider budget and could recreate hidden release state after the UI was removed. The cleanup must reduce provider work without deleting historical/provider-owned data or weakening unrelated concert, listening, artwork or identity behavior.

**Consequence:** This decision supersedes the active-product consequences of the 2026-08-02 **Spotify is the sole visible release-feed source** and **Alerts separate concerts from releases** decisions, and supersedes release-monitoring portions of the earlier scheduled-provider decisions where they would otherwise imply continuing release catalogue discovery. Historical release helpers/data may remain readable for backward compatibility, but scheduled v135 execution must not call MusicBrainz/Spotify release-catalogue discovery or generate release alerts. Structured scheduled research is forced through `DRY_RUN` at its preload boundary and provider work remains subject to the existing lease, UsageTracker, reuse, circuit-breaker and aggregate-only diagnostic controls. Spotify diagnostics count raw provider operations at the observer boundary; the currently instrumented Spotify public methods do not call one another, so nested helper double-counting is not present in the reviewed v135 implementation. Reintroducing a release feed, release alerts, or scheduled release discovery requires a new explicit decision and build. v135 does not authorize a production workflow, provider call, production-data write, deployment, merge or auto-merge.
