...
This first v109 slice adds a bounded maintenance runner around the merged Build A inventory and Build B enrichment engine. The runner accepts provider adapters, an explicit usage gate, persistence callback and resumable checkpoint state. Automated validation uses fictional fixtures only.
...
2. requires the usage gate to reserve that provider call before invoking any adapter;
3. executes exactly that planned operation through an injected provider adapter;
4. validates the provider response through the Build B resolver;
5. merges only derived track identity and Spotify metadata state;
6. requires persistence of the resulting state before another provider call may start;
7. replans from the newly persisted logical state rather than assuming a fixed queue.
...
The checkpoint records the logical run start/update time, completed operation keys and halt reason. It is audit/resume state, not an authority that can suppress an explicit Build B retry.
...
Persistence failure aborts the batch before another provider adapter is invoked. The caller must treat the failed in-memory snapshot as uncommitted and reload durable state before resuming.
