...
(function attachListeningIdentityPacingV105(root, factory) {
  ...
  function installRecordingOnlyUi() {
  ...
  function install() {
    let changed = false;
    if (root && !root.__bandmarkrIdentityPacingV105Installed && typeof root.fetch === 'function') {
      root.fetch = createPacedFetch(root.fetch.bind(root));
      root.__bandmarkrIdentityPacingV105Installed = true;
      changed = true;
    }
    if (typeof root?.document !== 'undefined') changed = installRecordingOnlyUi() || changed;
    return changed;
  }
