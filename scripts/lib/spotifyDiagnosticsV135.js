...
// Sanitized v135+ observability. Counts only lane/endpoint families and
// provider outcomes; never stores provider IDs, query text, URLs, tokens or payloads.
...
function freshDiagnostics(usage) {
  return {
    callsByLane: {}, callsByEndpoint: {}, outcomes: { attempted: 0 }, circuitEvents: [],
    circuitStart: circuitSnapshot(usage), circuitFinish: null,
  };
}
...
function ensure(usage) {
  ...
  if (!spotify.diagnostics || typeof spotify.diagnostics !== 'object' || Array.isArray(spotify.diagnostics)) spotify.diagnostics = freshDiagnostics(usage);
  ...
  if (!Number.isFinite(Number(spotify.diagnostics.outcomes.attempted))) spotify.diagnostics.outcomes.attempted = 0;
...
function recordOperation(usage, { lane, endpoint, before, beforeCircuit = null, result }) {
  ...
  lane = LANES.has(lane) ? lane : 'other'; endpoint = ENDPOINTS.has(endpoint) ? endpoint : 'other';
  count(diagnostics.outcomes, 'attempted');
  ...
  const outcome = outcomeKind(result); if (outcome) count(diagnostics.outcomes, outcome);
  const afterCircuit = circuitSnapshot(usage); diagnostics.circuitFinish = afterCircuit;
  if (beforeCircuit?.status !== 'open' && afterCircuit?.status === 'open') {
    diagnostics.circuitEvents.push({ lane, endpoint, callOrdinal: calls(usage), httpStatus: Number.isFinite(Number(result?.status)) ? Number(result.status) : null, reason: afterCircuit.reason, retryAfter: safeRetryAfter(result) });
    diagnostics.circuitEvents = diagnostics.circuitEvents.slice(-10);
  }
}

function wrap(spotify, name, lane, endpoint) {
  const original = spotify[name]; if (typeof original !== 'function') return;
  spotify[name] = async function v135Diagnosed(...args) {
    ...
    const before = calls(usage); const beforeCircuit = circuitSnapshot(usage);
    try { const result = await original.apply(this, args); recordOperation(usage, { lane, endpoint, before, beforeCircuit, result }); return result; }
    catch (error) { recordOperation(usage, { lane, endpoint, before, beforeCircuit, result: { kind: 'error', status: error?.status, retryAfter: error?.retryAfter, error: error?.code || 'request_failed' } }); throw error; }
  };
}
...
function installSpotifyDiagnosticsV135(spotify, UsageTracker = null) {
  ...
  if (UsageTracker && typeof UsageTracker.load === 'function' && !UsageTracker[LOAD_PATCH]) {
    ...
    UsageTracker.load = async function v135LoadWithFreshSpotifyDiagnostics(...args) { const usage = await originalLoad.apply(this, args); resetDiagnostics(usage); return usage; };
