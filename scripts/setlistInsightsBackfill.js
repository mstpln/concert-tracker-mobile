...
async function runSetlistInsightsBackfill({ maxConcerts = process.env.MAX_CONCERTS, forceRecalculate = process.env.FORCE_RECALCULATE, readConcerts = worker.readJson, readBands = worker.readJson, loadUsage = UsageTracker.load, process = processSetlistInsights, now = new Date(), log = console.log } = {}) {
