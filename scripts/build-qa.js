...
const root = process.cwd(); const out = path.join(root, 'dist'); const id = process.env.QA_BUILD_ID || process.env.GITHUB_SHA || process.env.CF_PAGES_COMMIT_SHA || 'local-qa';
...
html = html.replace('</head>', '<link rel="stylesheet" href="qa.css" /></head>').replace('</body>', '<script src="qa-fixtures.js"></script><script>window.__LIVEVAULT_QA_BUILD_ID__=' + JSON.stringify(id) + ';</script><script src="qa-bootstrap.js"></script></body>');
fs.writeFileSync(path.join(out, 'index.html'), html);
let sw = fs.readFileSync(path.join(root, 'service-worker.js'), 'utf8');
sw = sw.replace(/const CACHE_NAME = 'concert-tracker-shell-' \+ CACHE_NAME_LITERAL;/, `const CACHE_NAME = 'concert-tracker-qa-' + CACHE_NAME_LITERAL + '-${id.replace(/[^A-Za-z0-9_-]/g, '')}';`)
  .replace("  './version.js',", "  './version.js',\n  './qa-fixtures.js',\n  './qa-bootstrap.js',\n  './qa.css',");
fs.writeFileSync(path.join(out, 'service-worker.js'), sw);
fs.writeFileSync(path.join(out, 'robots.txt'), 'User-agent: *\nDisallow: /\n');
fs.writeFileSync(path.join(out, '_headers'), '/*\n  X-Robots-Tag: noindex, nofollow, noarchive\n  Referrer-Policy: no-referrer\n  X-Content-Type-Options: nosniff\n  Content-Security-Policy: default-src \'self\'; connect-src \'self\' https://qa.invalid; img-src \'self\' data: blob:; style-src \'self\' \'unsafe-inline\'; script-src \'self\'; worker-src \'self\' blob:\n');
