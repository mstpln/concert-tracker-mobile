...
for (const file of shell) {
  if (!fs.existsSync(path.join(root, file))) {
    throw new Error(`QA build is missing required shell file: ${file}`);
  }
}
...
fs.writeFileSync(
  path.join(out, 'qa-build-config.js'),
  `window.__LIVEVAULT_QA_BUILD_ID__ = ${JSON.stringify(id)};\n`
);
...
sw = sw
  .replace(
    ...
    `const CACHE_NAME = 'concert-tracker-qa-' + CACHE_NAME_LITERAL + '-${id}';`
  )
  .replace(
    "  './version.js',",
    "  './version.js',\n  './qa-fixtures.js',\n  './qa-build-config.js',\n  './qa-bootstrap.js',\n  './qa.css',"
...
for (const file of files) {
  if (forbidden.includes(path.basename(file)) || /ticket-files|\.pem$|\.key$/i.test(file)) {
    throw new Error(`QA build rejected unsafe file name: ${path.basename(file)}`);
  }
}

for (const src of [...html.matchAll(/(?:src|href)="([^"#?]+)"/g)].map((match) => match[1])) {
  if (!src.startsWith('http') && !fs.existsSync(path.join(out, src))) {
    throw new Error(`QA build generated missing shell reference: ${src}`);
  }
}

fs.writeFileSync(
  path.join(out, 'qa-build.json'),
  `${JSON.stringify(
    {
      synthetic: true,
      buildId: id,
      checksum: crypto.createHash('sha256').update(id).digest('hex').slice(0, 12),
    },
    null,
    2
  )}\n`
