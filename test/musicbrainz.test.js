...
test('31 Try again preserves rejected MBIDs and clears stale identity through production transformation', () => { const retry=retryIdentity({mbid:'old',artistName:'Old',confidence:99,matchMethod:'automatic',matchedAt:'then',score:99,matchReasons:['old'],rejectedCandidateMbids:['x'],reviewCandidates:[{mbid:'y'}]}, 'now'); assert.equal(retry.status,'pending'); assert.deepEqual(retry.rejectedCandidateMbids,['x']); assert.equal(retry.mbid,null); assert.equal(retry.confidence,null); assert.equal(retry.score,undefined); assert.equal(retry.matchReasons,undefined); assert.equal(musicbrainzEligible({musicbrainz:{status:'manual_rejected'}}), false); assert.equal(musicbrainzEligible({musicbrainz:retry}), true); });
