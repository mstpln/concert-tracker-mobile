...
test('3 ambiguous Spotify artist match requires review', async () => { const result = await spotify.resolveArtistIdentity({ band: band(), metadata: { artistName: 'The Example', aliases: [] }, usage: usage(), getToken: async () => 'x', fetchImpl: async () => ({ ok: true, json: async () => ({ artists: { items: [{ id: 'a', name: 'The Example' }, { id: 'b', name: 'The Example' }] } }) }) }); assert.equal(result.identity.status, 'needs_review'); });
...
test('7 ambiguous attraction requires review', async () => { const result = await ticketmaster.resolveAttractionIdentity({ band: band(), metadata: { artistName: 'The Example', aliases: [] }, usage: usage(), fetchImpl: async () => ({ ok: true, json: async () => ({ _embedded: { attractions: [{ id: 'a', name: 'The Example', classifications: [{ segment: { name: 'Music' } }] }, { id: 'b', name: 'The Example', classifications: [{ segment: { name: 'Music' } }] }] } }) }) }); assert.equal(result.identity.status, 'needs_review'); });
