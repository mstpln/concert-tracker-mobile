...
test('v81 source includes approved UI, bounded refresh and privacy boundaries', () => {
  ...
  assert.match(fix, /listeningVisitedBandId/);
  assert.match(fix, /setTimeout\(safeReload, fallbackDelay\);[\s\S]*await navigator\.serviceWorker\.getRegistration\(\)/);
  assert.doesNotMatch(`${ui}\n${fix}`, /localStorage\.clear|sessionStorage\.clear|indexedDB\.deleteDatabase/);
});

test('v81 correction artifacts keep contrast and continuity facts deterministic', () => {
  ...
  const decisions = fs.readFileSync(path.join(__dirname, '..', 'docs', 'LIVEVAULT_DECISIONS.md'), 'utf8');
  assert.match(css, /data-v81-year-genre="All"[^}]*background:var\(--year-chart-all\)[^}]*color:var\(--year-chart-all-text\)/);
  assert.ok(state.shellFiles.includes('listeningV81BootFix.js'));
  assert.match(decisions, /v81 timeframe and page-state defaults/);
  assert.equal(fs.existsSync(path.join(__dirname, '..', 'docs', 'LIVEVAULT_V81_DECISIONS.md')), false);
});
