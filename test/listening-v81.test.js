...
test('current build facts, contrast and shell entries remain deterministic', () => {
  ...
  const versionSource = fs.readFileSync(path.join(__dirname, '..', 'version.js'), 'utf8');
  const serviceWorkerSource = fs.readFileSync(path.join(__dirname, '..', 'service-worker.js'), 'utf8');
  const appVersion = versionSource.match(/const APP_VERSION\s*=\s*['"]([^'"]+)['"]/u)?.[1];
  const cacheVersion = serviceWorkerSource.match(/const CACHE_NAME_LITERAL\s*=\s*['"]([^'"]+)['"]/u)?.[1];
  ...
  assert.equal(state.appVersion, state.serviceWorkerCacheVersion);
  assert.equal(state.appVersion, appVersion);
  assert.equal(state.serviceWorkerCacheVersion, cacheVersion);
  ...
  assert.ok(state.shellFiles.includes('spotifyListeningMetadataV101.js'));
  ...
  assert.ok(state.shellFiles.includes('listeningIdentityPacingV105.js'));
  assert.ok(state.shellFiles.includes('listeningIdentityCompletionV104.js'));
  assert.ok(state.shellFiles.includes('listeningIdentityGroupingV104.js'));
});
