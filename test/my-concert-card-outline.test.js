...
test('My Concerts summary wrapper keeps equal outer gaps while preserving the card-to-card gap', () => {
  ...
  assert.match(app, /let html = '<div class="myconcerts-summary">';[\s\S]*?if \(past\.length > 0\) html \+= statsTeaserHtml[\s\S]*?html \+= countdownCardHtml\(EventModelV156\.nextEventPresentation\(upcoming\)\);[\s\S]*?html \+= '<\/div>';/);
  assert.match(css, /\.myconcerts-summary \{[\s\S]*?--myconcerts-summary-outer-gap: 28px;[\s\S]*?margin-top: calc\(var\(--myconcerts-summary-outer-gap\) - 12px\);[\s\S]*?margin-bottom: var\(--myconcerts-summary-outer-gap\);[\s\S]*?\}/);
