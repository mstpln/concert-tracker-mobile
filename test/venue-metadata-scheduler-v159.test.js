...
function concert(id, venue, city, country, date, attending = true) {
  return { id, venue, city, country, date, attending };
}
...
test('write retries one ETag conflict against latest venue state', async () => {
  ...
  await Scheduler.writeWithOneConflictRetry(client, [update]);
