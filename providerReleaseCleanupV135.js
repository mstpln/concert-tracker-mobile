...
(function attachProviderReleaseCleanupV135(root) {
  function renderAlertsOnly() {
    ...
    container.innerHTML = alerts.length ? alerts.map(alertRowHtml).join('') : '<p class="screen-empty">No new concerts found in the last 90 days.</p>';
    container.querySelectorAll('.row-card[data-band-id]').forEach((row) => row.addEventListener('click', () => openProfile(row.dataset.bandId, { selectedTab: 'alerts' })));
  }
  ...
  function apply() {
    if (typeof renderNewsScreen === 'function') renderNewsScreen = renderAlertsOnly;
